...
import { Mail, ArrowLeft, MailCheck, CodeXml, Zap, Cloud } from "lucide-react";
  ...
  async function handleResetPassword(e: React.FormEvent) {
    ...
    if (error) { 
      if (error.message.toLowerCase().includes("rate limit")) {
        setError("You've requested too many reset links. Please wait a few minutes and try again.");
      } else {
            ...
            <motion.div variants={containerVariants} initial="hidden" animate="show" className="w-full max-w-[420px]">
              ...
              {success ? (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center space-y-4 pt-8 pb-4">
                  ...
                  <p className="text-[#6B7280] px-4">We sent a password reset link to <span className="font-medium text-[#111827]">{email}</span></p>
              ...
              <motion.p variants={itemVariants} className="text-center mt-6 lg:mt-8 text-[14px] text-[#111827]">
                ...
                <Link href="/login" className="font-bold text-[#5742FF] hover:underline inline-flex items-center gap-1">
                  Back to log in
                </Link>
              </motion.p>
            </motion.div>
